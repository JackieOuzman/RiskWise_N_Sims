...
merged_files_Daily <- merged_files_Daily %>% 
  mutate(Yield_Moisture_corrected = case_when(
    ...
    Year == 2018 ~ Yield*12.5, #W
    Year == 2019 ~ Yield*12.5, #C
    Year == 2020 ~ Yield*12.5, #W
    Year == 2021 ~ Yield*12.5, #B
    Year == 2022 ~ Yield*12.5, #W
    Year == 2023 ~ Yield*14, #Chick
    Year == 2024 ~ Yield*12.5, #B
    
    
    )) %>% 
  
  mutate(Biomass_Moisture_corrected = case_when(
    Year == 2018 ~ Biomass*12.5,
    Year == 2019 ~ Biomass*12.5,
    Year == 2020 ~ Biomass*12.5,
    Year == 2021 ~ Biomass*12.5,
    Year == 2022 ~ Biomass*12.5,
    Year == 2023 ~ Biomass*14,
...
# Plot --------------------------------------------------------------------
Location_of_Sims<- "X:/Riskwi$e/Cuyro/3_Sims_post_Nov2024/"
NextG_location <- "Cuyro_5_Rotation_N_bank.apsimx"
...
plot2 <- merged_files_Daily %>%
  ...
  theme_bw() +
  labs(
    title = "Biomass* 12.5% for wheat, barley and canola and 14% for chickpeas Cuyro Sims ",
    #subtitle =  ",
    colour = "",
    x = "Year",
    y = "",
...
plot5 <- merged_files_Daily %>%
 ...
 # filter(zadok_stage >= 0) %>%
  ...
  theme_bw() +
  labs(
    title = "Yield * 12.5% and 14%. Cuyro Sims ",
    subtitle = "2023 has no trial harvest date",
    colour = "",
    x = "Year",
    y = "",
    caption = paste0(
...
write.csv(N_response_sim_Trail_long ,
          "X:/Riskwi$e/Curyo/3_Sims_post_Nov2024/Results/N_response_sim_Trial_long.csv", row.names = FALSE )
...
N_Response_plot <- N_response_sim_Trail_long %>% 
  ...
  theme_bw() +
  labs(
    title = "Response curve yield *12.5% and 8%. Cuyro",
    colour = "",
    x = "InCropFert",
    y = "",
    caption = paste0(
...
sowing_dates <- data.frame(date = as.Date(c(
  ...
  "2021-05-14",
  "2022-05-19",
  "2023-05-01", # guess nothing in Melb uni DB
  "2024-05-01"
)))
sowing_dates
...
Fert_dates <- data.frame(date = as.Date(c(
  "2018-05-14",
  "2018-08-28", #in season
  
  "2019-04-29",
  "2019-06-28",#in season
  
  "2020-05-16",
  "2020-07-23",#in season
  
  "2021-05-14",
  "2021-07-23",#in season
  
  "2022-05-19",
  "2022-07-08",#in season
  
  "2023-05-01", # guess nothing in Melb uni DB
  
  "2024-05-01",
  "2024-06-24" #in season
  )))
Fert_dates
