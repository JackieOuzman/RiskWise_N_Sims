...
path <- "X:/Riskwi$e/Temora/3_Sims_post_Nov2024/Results/"
...
merged_files_Daily <- merged_files_Daily %>% 
  mutate(Yield_Moisture_corrected = case_when(
    Year == 2022 ~ Yield*12.5, #wheat
    Year == 2023 ~ Yield*8 #canola
    #Year == 2024 ~ Yield*8
  )) %>% 
...
# Plot --------------------------------------------------------------------
Location_of_Sims<- "X:/Riskwi$e/Temora/3_Sims_post_Nov2024/"
NextG_location <- "Temora_5_Rotation_N_bank.apsimx"
...
plot2 <- merged_files_Daily %>%
  ...
  theme_bw() +
  labs(
    title = "Biomass*12.5 in 2022 & *8 in 2023. Temora Sims ",
    #subtitle =  ",
    colour = "",
    x = "Year",
    y = "",
...
plot5 <- merged_files_Daily %>%
 ...
 # filter(zadok_stage >= 0) %>%
  ...
  theme_bw() +
  labs(
    title = "Yield *12.5 in 2022 & *8 in 2023 Temora Sims ",
   # subtitle = "No modifcation to organic matter",
    colour = "",
    x = "Year",
    y = "",
...
N_Response_plot <- N_response_sim_Trail_long %>% 
  ...
  theme_bw() +
  labs(
    title = "Response curve Temora (Yield corrected *12.5 in 2022 & *8 in 2023)",
    colour = "",
    x = "InCropFert",
    y = "",
    caption = paste0(
...
Fert_dates <- data.frame(date = as.Date(c(
  "2022-05-10", #sowing
  "2022-06-27", #inseason 1
  #"2023-07-03", #inseason 2
  "2023-04-21", #sowing
  "2023-06-15"  #inseason 1
  )))
Fert_dates
