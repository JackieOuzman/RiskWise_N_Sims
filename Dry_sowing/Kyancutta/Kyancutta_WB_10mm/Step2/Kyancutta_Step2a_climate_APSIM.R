...
Kyancutta_Daily_APSIM <- read_csv("X:/Riskwi$e/Dry_sowing/Kyancutta/APSIM_runs/Kyancutta_Water_balance_v5 10mm.Daily.csv", 
...
#check <- Kyancutta_Daily_APSIM %>%  filter(between(Wheat.Phenology.Stage, 6.49, 9.5))
#rm(check)
...
Kyancutta_Daily_APSIM$Clock.Today <- as.Date(Kyancutta_Daily_APSIM$Clock.Today)
Kyancutta_APSIM_Met <- left_join(Kyancutta_Daily_APSIM,met_frost, by = join_by("Clock.Today" == "date"))
...
Kyancutta_APSIM_Met <- Kyancutta_APSIM_Met %>% mutate(
  start_end_GS = case_when(
    ...
    TRUE ~ NA))
### fill the blanks
Kyancutta_APSIM_Met <- Kyancutta_APSIM_Met %>% fill(start_end_GS) %>%
  mutate(
    season = case_when(
...
Kyancutta_APSIM_Met <- Kyancutta_APSIM_Met %>% select(-start_end_GS)  
str(Kyancutta_APSIM_Met)
...
Kyancutta_APSIM_Met_gs <- Kyancutta_APSIM_Met %>% filter(season == "gs")
...
unique(Kyancutta_APSIM_Met_gs$frost_most_Sensitive_period)   
Kyancutta_APSIM_Met_gs <- Kyancutta_APSIM_Met_gs %>% 
  mutate(
...
#test
str(Kyancutta_APSIM_Met_gs)
test <- Kyancutta_APSIM_Met_gs %>% filter(year == 2024) %>%  filter(Sowing_date== "10-may") %>% 
...
write.csv(Kyancutta_APSIM_Met_gs ,
          "X:/Riskwi$e/Dry_sowing/Kyancutta/Results/WB_10/WB10_Dry_sowing_Kyancutta_factor_with_met_18170_v2_gs.csv", row.names = FALSE )
...
names(Kyancutta_APSIM_Met_gs)
str(Kyancutta_APSIM_Met_gs)

days_GS <- Kyancutta_APSIM_Met_gs %>% 
  group_by(year,Sowing_date ) %>% 
  count(season)
days_GS <- days_GS %>%  rename(days_GS = n)
days_GS

frost_event_count_year <- Kyancutta_APSIM_Met_gs %>% 
  group_by(year,Sowing_date) %>% 
  count(frost_event)
...
frost_in_Sensitive_period_count_year <- Kyancutta_APSIM_Met_gs %>% 
  group_by(year,Sowing_date) %>% 
...
frost_in_most_Sensitive_period_count_year <- Kyancutta_APSIM_Met_gs %>% 
  group_by(year,Sowing_date) %>% 
...
path_saved_files
ggsave(plot = plot1,
       filename = paste0(path_saved_files,"/WB_10_Climate_Frost_days_Kyancutta_v2", ".png" ),
       width = 20, height = 12, units = "cm")

ggsave(plot = plot2,
       filename = paste0(path_saved_files,"/WB10_Climate_Frost_days_Kyancutta_10-May_v2", ".png" ),
       width = 20, height = 12, units = "cm")

write_csv(summary_frost_details,
          file = paste0(path_saved_files,"/WB10_summary_frost_details_Kyancutta_v2", ".csv"))
