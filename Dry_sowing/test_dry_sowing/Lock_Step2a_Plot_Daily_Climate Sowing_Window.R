...
Dry_sowing_Lock_factor_with_met <- read_csv("X:/Riskwi$e/Dry_sowing/Lock/Dry_sowing/Results/Dry_sowing_Lock_factor_with_met_180462022_2024.csv")
...
Sim_met_apr102024 <- Dry_sowing_Lock_factor_with_met %>% 
  filter(year == 2024) %>% 
  filter(Sowing_date == "1-apr")
Sim_met_may102024

unique(Sim_met_apr102024$Wheat.Phenology.CurrentStageName)
phenology_stages <- Sim_met_apr102024 %>% filter(!is.na(Wheat.Phenology.CurrentStageName)) %>% 
  select(Clock.Today,Wheat.Phenology.CurrentStageName )
phenology_stages

str(Sim_met_apr102024)
unique(Sim_met_apr102024$frost_Sensitive_period)
Frost_senstive_period <- Sim_met_apr102024 %>% 
  select(Clock.Today, frost_Sensitive_period ) %>% 
  filter(frost_Sensitive_period == "frost_sensitive_period" )
Frost_senstive_period

Frost_most_senstive_period <- Sim_met_apr102024 %>% 
...
str(Sim_met_apr102024)
names(Sim_met_apr102024)

PAW_sowing_Emer <- Sim_met_apr102024 %>% 
  filter(Wheat.Phenology.CurrentStageName== "Sowing"|Wheat.Phenology.CurrentStageName== "Emergence") %>% 
  select(Clock.Today, Wheat.Phenology.CurrentStageName, PAW_1:PAW_SUM)
PAW_sowing_Emer$PAW_SUM



plot1 <- Sim_met_apr102024 %>% 
  ...
  geom_point()+
  annotate("text", x= as.Date("2024-05-01"), y=15, label="PAW sum Sowing = 0.278", color = "black", size=5)+ 
  annotate("text", x= as.Date("2024-05-01"), y=14, label="PAW sum Emergence  = 0.267", color = "black", size=5 )+
  theme_classic()+
...
ggsave(plot = plot1,
       filename = paste0(path_saved_files,"/MinTemp_vs_days_Lock2023_2022_2024", ".png" ),
