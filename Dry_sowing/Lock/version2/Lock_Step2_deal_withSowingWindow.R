## This is an extra step to deal with permetations created when factorising a sowing rule with a window
...
### This is a csv file with factor and permetation 
Dry_sowing_Lock_factor_KPV2_Daily <- read_csv("X:/Riskwi$e/Dry_sowing/Lock/Dry_sowing/Dry_sowing_Lock_factor_KPV2 Small_2022_2024.Daily.csv", 
                                                              col_types = cols(Clock.Today = col_date(format = "%Y-%m-%d")))


names(Dry_sowing_Lock_factor_KPV2_Daily)
str(Dry_sowing_Lock_factor_KPV2_Daily)


Dry_sowing_Lock_factor_KPV2_Daily <- Dry_sowing_Lock_factor_KPV2_Daily %>% select(
  CheckpointName,
  SimulationName,
  StartDate,
  EndDate,
...
Dry_sowing_Lock_factor_KPV2_Daily <- Dry_sowing_Lock_factor_KPV2_Daily %>% 
  mutate(Sowing_window = paste0(StartDate, "-",EndDate ))
unique(Dry_sowing_Lock_factor_KPV2_Daily$Sowing_window)
# what I want to keep is:
 sowing_window_retain <- c(
#                           "1-apr-5-apr",
#                           "5-apr-10-apr",
#                           "10-apr-15-apr",
#                           "15-apr-20-apr",
...
Dry_sowing_Lock_factor_with_met <- Dry_sowing_Lock_factor_with_met %>% 
  mutate(
    frost_in_Sensitive_period = 
      case_when(
      frost_event == "frost" | 
        frost_Sensitive_period == "frost_sensitive_period"~ "frost_in_Sensitive_period",
...
plot1 <- summary_frost_details %>% filter(grouping == "frost" ) %>% 
  ...
  labs(title = "Climate station Lock 18046",
       ...
       caption = "Note; Note; 9 frost events occured in most sensitive period over 3 years")
plot1
...
plot2 <- summary_frost_details %>% 
  ...
  labs(title = "Climate station Lock 18046",
       ...
       x = "years",
       y = "Percentage of days classified as frost (-4 to 1)",
       caption = "Note; 9 frost events occured in most sensitive period over 3 years")
plot2
...
write_csv(summary_frost_details,
          file = paste0(path_saved_files,"/summary_frost_details_Lock_v2", ".csv"))
