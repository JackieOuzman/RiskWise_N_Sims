...
str(phenology_wide)
phenology_wide$year <- as.factor()

plot1 <- phenology_wide %>% 
  ggplot(aes(x = as.factor(year), from_sowing_to_emergence))+
  geom_point()+
  facet_wrap(. ~ Sim)+ 
  theme_bw()+
...
plot4 <- phenology_wide %>% 
  ggplot(aes(x =as.factor(year), from_sowing_to_emergence, group = Sim, colour = Sim))+
  geom_point(size= 4)+
 # facet_wrap(. ~ Sim)+ 
  theme_bw()+
  #theme(legend.position = "none")+
  labs(title = "Number of days vs years",
       #subtitle = "Days from sowing to harvest only. Fixed sowing dates 10-May",
       x = "years",
       y = "number of days from sowing to emeregnace"
  )
...
plot5 <- phenology_wide %>% 
  ggplot(aes(x =as.factor(year), Yield_Adj_t_ha, group = Sim, colour = Sim))+
  geom_point(size = 4)+
  # facet_wrap(. ~ Sim)+ 
  theme_bw()+
  #theme(legend.position = "none")+
  labs(title = "Yield over years",
       #subtitle = "Days from sowing to harvest only. Fixed sowing dates 10-May",
       x = "years",
       y = "Yield t/ha"
  )
...
ggsave(plot = plot4,
       filename = paste0(path_saved_files,"/Number of days from sowing to emergence vs years_Lock", ".png" ),
       width = 20, height = 12, units = "cm")
ggsave(plot = plot5,
       filename = paste0(path_saved_files,"/yield vs yrs by sowin date Lock", ".png" ),
       width = 20, height = 12, units = "cm")
...
Dry_sowing_Lock_factor_with_met
