...
Dry_sowing_Lock_factor_with_met <- read_csv("X:/Riskwi$e/Dry_sowing/Lock/Results/short_window_10mm/Short_window_Dry_sowing_Lock_factor_with_met_18046_v2_gs.csv")
...
min(Dry_sowing_Lock_factor_with_met$Clock.Today)
max(Dry_sowing_Lock_factor_with_met$Clock.Today)
number_potentail_years <- max(Dry_sowing_Lock_factor_with_met$year ) - 
                          min(Dry_sowing_Lock_factor_with_met$year )
################################################################################
### not every years triggered a sowing event
Dry_sowing_Lock_factor_with_met %>% distinct(Wheat.Phenology.CurrentStageName)

sowing_germination_event_trigger <- Dry_sowing_Lock_factor_with_met %>% 
...
years_germ
################################################################################
path_saved_files <- file_path_input_data<-file.path("X:","Riskwi$e", "Dry_sowing", "Lock", "Results", "short_window_10mm")
ggsave(plot = years_germ,
       filename = paste0(path_saved_files,"/Short_window_years_Optimal germination conditions was triggered Lock", ".png" ),
...
start_dates <- Dry_sowing_Lock_factor_with_met %>% distinct(Sowing_date                     )
start_dates_1 <-dplyr::pull(start_dates, Sowing_date)

max(Dry_sowing_Lock_factor_with_met$year)
min(Dry_sowing_Lock_factor_with_met$year)
...
ggsave(plot = plot2_box,
       filename = paste0(path_saved_files,"/Small_window_Box_yield_vs_drySowing_dates_dummy_zero_ylds_Lock", ".png" ),
       width = 20, height = 12, units = "cm")
...
ggsave(plot = plot3_mean_yld,
       filename = paste0(path_saved_files,"/Small_window_mean_yield_vs_drySowing_dates_dummy_zero_ylds_Lock", ".png" ),
       width = 20, height = 12, units = "cm")
